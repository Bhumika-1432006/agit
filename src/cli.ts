...
function cmdStats(opts: Opts): number {
  ...
  console.log(
    `\n  totals: in=${totals.inputTokens.toLocaleString("en-US")} out=${totals.outputTokens.toLocaleString("en-US")} (${totals.apiMessages.toLocaleString("en-US")} API calls across ${sessionsRead} sessions)`,
  );
  return 0;
}
